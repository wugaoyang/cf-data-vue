...
function createAxios<Data = any, T = ApiPromise<Data>>(axiosConfig: AxiosRequestConfig, options: Options = {}, loading: LoadingOptions = {}): T {
    ...
    if(axiosConfig.url?.includes("upload")){
        headers = null
    }
    ...
    Axios.interceptors.request.use(
        (config) => {
            ...
            if (config.headers) {
                // adminInfo.setToken("");
                // (config.headers as anyObj)["Content-Type"] = "application/x-www-form-urlencoded;charset=UTF-8";
...
// 响应拦截
    Axios.interceptors.response.use(
        (response) => {
            ...
            const code1 = response.data.code;// 关闭loading
            if (response.config.responseType == 'json') {
                if (response.data && code1 !== '00000') {
                    if (code1 === ResultEnum.TOKEN_INVALID) {
                        reLogin();
                        return
                    }
                ...
                } else if ((options.showSuccessMessage && response.data && (code1 == 1 || code1 == '00000'))) {
                    ElNotification({
            ...
            if (error.response && error.response.data.code == '409') {
                if (!window.tokenRefreshing) {
                    ...
                    return refreshToken()
                        .then((res) => {
                            if (res.code == '00000') {
                                adminInfo.setToken(res.data.accessToken, 'auth')
                                adminInfo.setToken(res.data.refreshToken, 'refresh')
                                response.headers.batoken = `${res.data.accessToken}`
                                window.requests.forEach((cb) => cb(res.data.accessToken, 'Authorization'))
                                window.requests = []
                ...
                if (error.response && error.response.data.code == ResultEnum.TOKEN_INVALID) {
                    reLogin();
                    return
                }
            }
...
function httpErrorStatusHandle(error: any) {
    ...
    if (error.message.includes('Network'))
        message = 'axios.Server exception!'
